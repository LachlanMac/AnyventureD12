...
const SongsTab: React.FC<{ characterId: string }> = ({ characterId }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/characters/${characterId}/songs`, {
    ...
    <div>
      ...
      {loading ? (
        ...
        <Card variant="default">
          <CardBody>
            <div style={{ color: 'var(--color-sunset)', textAlign: 'center' }}>{error}</div>
          </CardBody>
        </Card>
      ) : songs.length === 0 ? (
        <Card variant="default">
                  ...
                  >
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.25rem' }}>
                        ...
                        >
                          difficulty: {s.songId?.difficulty}
                        </span>
                        {s.songId?.magical && (
                          <span
                            style={{
                              ...
                              border: '1px solid rgba(215,183,64,0.6)',
                              color: '#f5d46a',
                            }}
                          >
                            magical
                    ...
                    <Link to={`/songs/${s.songId?._id || s.songId}`} style={{ color: '#fff' }}>
                      View
                    </Link>
                  </div>
                </CardHeader>
                <CardBody>
                  ...
                  {s.songId?.description && (
                    <div style={{ marginTop: '0.75rem' }}>
                      <p style={{ color: 'var(--color-cloud)', fontSize: '0.9rem' }}>
                        {s.songId.description}
                      </p>
                    </div>
                  )}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        {s.songId?.harmony_1 && (
                          ...
                          >
                            ...
                            >
                              Talent: {s.songId.harmony_1.instrument}
                            </span>
                            <span style={{ color: 'var(--color-cloud)' }}>
                              {s.songId.harmony_1.effect}
                            </span>
                          </div>
                        )}
                        {s.songId?.harmony_2 && (
                          ...
                          >
                            ...
                            >
                              Talent: {s.songId.harmony_2.instrument}
                            </span>
                            <span style={{ color: 'var(--color-cloud)' }}>
                              {s.songId.harmony_2.effect}
                            </span>
                          </div>
                        )}
                      </div>
