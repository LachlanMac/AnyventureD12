...
const SongsPage: React.FC = () => {
  ...
  const learnedIds = useMemo(
    () =>
      new Set(
        (character && 'songs' in character ? (character as any).songs : []).map((s: any) =>
          typeof s.songId === 'string' ? s.songId : s.songId?._id
        )
      ),
    [character]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (magicalFilter !== 'all')
      list = list.filter((s: any) => s.magical === (magicalFilter === 'yes'));
    if (search.trim()) {
  ...
  const handleRemove = async (songId: string) => {
    ...
    try {
      setActionLoading(true);
      await songsApi.removeFromCharacter(characterId, songId);
      await refetchCharacter();
    } finally {
      setActionLoading(false);
    }
  ...
  if (error || !character)
    return (
      <div className="container mx-auto px-4 py-8">
        <ErrorState
          title="Failed to Load Character Songs"
                      ...
                      >
                        ...
                        >
                          ...
                          <div>
                            {learned ? (
                              <span
                                style={{ color: 'var(--color-metal-gold)', fontSize: '0.75rem' }}
                              >
                                learned
                              </span>
                            ) : (
                ...
                <>
                  ...
                  >
                        ...
                        {selected.magical && (
                          <span
                            style={{
                              ...
                              padding: '0.125rem 0.5rem',
                              borderRadius: '9999px',
                              backgroundColor: 'rgba(215,183,64,0.15)',
                    ...
                    {!isLearned(selected._id) ? (
                      <Button
                        variant="accent"
                        onClick={() => handleAdd(selected._id)}
                  ...
                  {selected.description && (
                    <div style={{ marginTop: '1rem' }}>
                      <h4 style={{ color: 'var(--color-white)', fontWeight: 'bold' }}>
                        Description
                      </h4>
                    ...
                    <div style={{ marginTop: '1rem' }}>
                      ...
                      {selected.harmony_1 && (
                        ...
                        >
                          ...
                          >
                            Talent: {selected.harmony_1.instrument}
                          </span>
                          <span style={{ color: 'var(--color-cloud)' }}>
                            {selected.harmony_1.effect}
                          </span>
                        </div>
                      )}
                      {selected.harmony_2 && (
                        ...
                        >
                          ...
                          >
                            Talent: {selected.harmony_2.instrument}
                          </span>
                          <span style={{ color: 'var(--color-cloud)' }}>
                            {selected.harmony_2.effect}
                          </span>
                        </div>
                      )}
                    </div>
